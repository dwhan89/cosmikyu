...
if mode == "full":
    ...
    tertiary_idx =  np.load("/home/dwhan89/workspace/cosmikyu/data/sehgal/ps_dist_tertiary_train010121_tszflux_sortedidx.npy")
...
#SDS_target = datasets.SehgalDataSet(sehgal_dir, "train141020", transforms=[SDN, RF], dummy_label=False, dtype=np.float64,  subset=primary_idx)
#SDS_input = datasets.SehgalDataSet(sehgal_dir, "train_tertiary191120", transforms=[SDN, RF], dummy_label=False, dtype=np.float32, subset=tertiary_idx)

SDS_target = datasets.SehgalDataSet(sehgal_dir, "train281220_fromcat", transforms=[SDN, RF], dummy_label=False, dtype=np.float64,  subset=primary_idx)
SDS_input = datasets.SehgalDataSet(sehgal_dir, "train_tertiary281220_fromcat", transforms=[SDN, RF], dummy_label=False, dtype=np.float32, subset=tertiary_idx)

DSJ = datasets.DataSetJoiner([SDS_input,SDS_target],  dtype=np.float64, shape=(10, 128, 128), dummy_label=True, shuffle=True)
...
nconv_layer = 5
experiment_id = "f833f5c803c148bd91d25de4f49cee81"#"a7f687e4e2fa4dd3b02096c56b850527"
