...
def init(switch=False):
    ...
    if not switch:
        print("WARNING: MPI is turned off by default. Use mpi.init(switch=True) to initialize MPI. MPI is turned off")
        return exit_code
    else:
        _switch = True
    ...
    try:
        ...
        sys.stderr.write(
            "IMPORT ERROR: " + __file__ + " (" + str(exc) + "). Could not load mpi4py. MPI will not be used.\n")
    ...
    if rank != dest:
        ...
        for sender in senders:
            ...
            if type(data) == dict:
                ...
                utils.merge_dict(data, recv_data)
